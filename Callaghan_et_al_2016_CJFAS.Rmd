...
library(MASS)
df<-fish[fish$DATETIME>="2013-08-15" & fish$DATETIME<="2013-10-15",c("SEX","DEPTH","DATETIME")]
df<-df[!is.na(df$DEPTH),]
df$DATETIME<-as.numeric(df$DATETIME)
# Default call 
k <- kde2d(df$DATETIME, df$DEPTH, n=50)
image(k,col=grey.colors(100),ylim=c(32,0))
```
...
```{r  bottomcontour, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=figs("bottomcontour",caption= "Kernel density estimate of bottom contour depth occupied by male (blue) and female (red) lake trout in Alexie Lake, NWT, between August 30 and September 30, 2013.")}
...
```{r shoredist,echo=FALSE, message=FALSE, fig.cap=figs("shoredist",caption="Kernel density estimate (KDE) of  distance to nearest shore occupied by male (blue) and female (red) lake trout in Alexie Lake, NWT, between August 30 and September 30, 2013.")}
...
```{r sitebar, echo=FALSE,fig.height=11, fig.width=8.5, fig.cap=figs("sitebar")}
data<-ltspawnclust %>%
  filter(MONTH==9)
data<-table(data$YEAR,data$SITE)
...
```{r spawntime, echo=FALSE, message=FALSE, warning=FALSE ,fig.height=9, fig.width= 8.5, fig.cap=figs("spawntime", caption="Thinking about changing colours of different years to the same as the stacked bar chart")}
require(scales)
...
lines(c(peakdates.13$START-12*60*60,peakdates.13$START-12*60*60),c(0,30))
lines(c(peakdates.13$END+12*60*60,peakdates.13$END+12*60*60),c(0,30))
...
lines(c(peakdates.14$START-12*60*60,peakdates.14$START-12*60*60),c(0,10))
lines(c(peakdates.14$END+12*60*60,peakdates.14$END+12*60*60),c(0,10))
...
Using our spatial temporal clustering algorithm, we estimated that `r length(unique(ltspawnclust$TRANSMITTER[ltspawnclust$MONTH==9]))` lake trout formed spawning clusters at `r length(unique(ltspawnclust$SITE[ltspawnclust$MONTH==9]))` unique sites during the 20120--2014 spawning seasons (~September; `r figs("sitebar", display="cite")`). Site use varied among sites with a mean of 9.1 spawning clusters and ranging from 1--92, as well as among years with 23 lake trout creating 567 spawning clusters in 2012, 18 lake trout creating 309 clusters in 2013 and 6 lake trout creating 53 clusters in 2014 (`r figs("spawntime",display="cite")`). Fifty percent of spawning clusters occured among 15 sites, and 95% occuring among 71 sites (`r figs("sitebar", display="cite")`). 

Spawning sites were distributed throughout the nearshore region of Alexie Lake covering an area of `r round(all_mcp$area,digits=2)` 3km^2^ (based on MCP).

maybe talk about mcp among years?

maybe talk about distance between sites?

maybe talk about distance between sotes among years?    


```{r clustbub12,echo=FALSE,message=FALSE,warning=FALSE,fig.height=9.5 ,fig.width=8, fig.cap=figs("clustbub12", caption="Thinking about changing colours of different years to the same as the stacked bar chart")}
...
###Timing of spawning  
  ...
  Peak spawning generally commenced September 10 (`r figs("spawntime", display="cite")`), lasting `r as.numeric(difftime(peakdates.12$END,peakdates.12$START,,units="days"))` days in 2012 (`r format(peakdates.12$START,"%B %e")`--`r format(peakdates.12$END,"%B %e")`); `r as.numeric(difftime(peakdates.13$END,peakdates.13$START,,units="days"))` days in 2013 (`r format(peakdates.13$START,"%B %e")`--`r format(peakdates.13$END,"%B %e")`); and `r as.numeric(difftime(peakdates.14$END,peakdates.14$START,,units="days"))` days in 2014 (`r format(peakdates.14$START,"%B %e")`--`r format(peakdates.14$END,"%B %e")`). Surface temperature at the commencement of peak spawn was similar in 2012 (`r round(dailytemp$TEMP[dailytemp$DATE==peakdates.12$START], digits=1)`&deg;C) and 2013 (`r round(dailytemp$TEMP[dailytemp$DATE==peakdates.13$START], digits=1)`&deg;C) but was lowest in 2014 (`r round(dailytemp$TEMP[dailytemp$DATE==peakdates.14$START], digits=1)`&deg;C). Similarly, the completion of peak spawn in 2012 and 2013 occured at close temperatures of `r round(dailytemp$TEMP[dailytemp$DATE==peakdates.12$END], digits=1)`&deg;C and `r round(dailytemp$TEMP[dailytemp$DATE==peakdates.13$END], digits=1)`&deg;C respectively, but several degrees lower in 2014 (`r round(dailytemp$TEMP[dailytemp$DATE==peakdates.14$END], digits=1)`&deg;C). 


```{r eval=FALSE, echo=FALSE}

 The maximum number of spawning clusters (`r max(spawn.num$FREQ)`) occured on `r format(peakdays[1],"%B %e")`. The majority of all spawning clusters (`r round(dim(spawn[spawn$DATE>=peakstart & spawn$DATE<=peakend,])[1]/dim(spawn)[1]*100)`%) and high activity spawning clusters (`r round(dim(spawn[spawn$DATE>=peakstart & spawn$DATE<=peakend & spawn$SPAWN==1,])[1]/dim(spawn[spawn$SPAWN==1,])[1]*100)`%) occurred during peak spawn. The frequency distributions of total, high activity, and low activity spawning cluster were similar (Fig. 8). Peak spawning for male and female lake trout occurred at similar dates, with peaks occurring on `r format(malepeak, "%B %e")` and `r format(femalepeak, "%B %e")`, respectively  (Fig. 9). Males collectively remained on the spawning shoals for maximum of `r as.numeric(mdays)` consecutive days while females collectively spent a maximum of `r as.numeric(fdays)` consecutive days.
...
b<-chisq.test(table(ltspawnclust3$TIME[ltspawnclust3$SPAWN==0]))
c<-chisq.test(table(ltspawnclust3$TIME))
```



```{r spawnday ,echo=FALSE, message=FALSE,fig.width=8,fig.height=7,fig.cap=figs("spawnday", caption=" Lake trout spawning cluster formation start times over the duration of the 2013 spawning season (left panel), and summarized as total counts per hour (right panel), in Alexie Lake, NWT. Both low activity (open circles) and high activity (closed circles) are displayed in both panels. Light grey polygons represent dawn and dusk, dark grey polygons represent night time and vertical black lines represent the onset and conclusion of peak spawning. The lines in the right panel represent a LOESS trendline with 95% confidence interval error bars.")}
#quartz()
...
mod2<-lme(sqrt(STEPLENGTH)~SEX+cDAY+I(cDAY^2)+YEAR, 
          random=~1|TRANSMITTER, correlation=corAR1(form=~1|TRANSMITTER) ,
...
mod3<-lme(sqrt(STEPLENGTH)~cDAY+I(cDAY^2), random=~1|TRANSMITTER, correlation=corAR1(form=~1|TRANSMITTER) ,data=data, method="REML")
...
#check assumptions
#heterogeneity
#plot(fitted(best.dd), residuals(best.dd, type="normalized"), xlab = "Fitted Values", ylab = "Residuals")
#abline(h = 0, lty = 2)
#lines(smooth.spline(fitted(best.dd), residuals(best.dd, type="normalized")),col="red")
#normality
#hist(residuals(best.dd,type="normalized"))
#independence
#acf(residuals(best.dd,type="normalized"))
...
mod3<-lme(PERSINDEX~SEX+cDAY, random=~1|TRANSMITTER, correlation=corAR1(form=~1|TRANSMITTER) ,data=data, method="REML")
...
#check assumptions
#heterogeneity
#plot(fitted(best.pi), residuals(best.pi, type="normalized"), xlab = "Fitted Values", ylab = "Residuals")
#abline(h = 0, lty = 2)
#lines(smooth.spline(fitted(best.pi), residuals(best.pi, type="normalized")),col="red")
#normality
#hist(residuals(best.pi,type="normalized"))
#independence
#acf(residuals(best.pi,type="normalized"))
...
mod3<-lme(log(ACCELMEAN)~SEX+I(cDAY^2), random=~1|TRANSMITTER, correlation=corAR1(form=~1|TRANSMITTER) ,data=data.ac, method="REML")

#Anova(mod3)
#mod3<-update(mod2,.~.-YEAR)
#Anova(mod3)
...
#check assumptions
#heterogeneity
#plot(fitted(best.ac), residuals(best.ac, type="normalized"), xlab = "Fitted Values", ylab = "Residuals")
#abline(h = 0, lty = 2)
#lines(smooth.spline(fitted(best.ac), residuals(best.ac, type="normalized")),col="red")
#normality
...
mf<-new.daily %>%
    filter(!TRANSMITTER=="LT-32" & MONTH==9 & !SEX=="U") %>%
  group_by(SEX) %>%
  summarise(STEPLENGTH=mean(STEPLENGTH, na.rm=TRUE),
            PERSINDEX=mean(PERSINDEX, na.rm=TRUE),
            ACCEL=mean(ACCELMEAN,na.rm=TRUE))
